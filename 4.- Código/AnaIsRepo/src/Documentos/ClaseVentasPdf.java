...
public class ClaseVentasPdf {
    ...
    public void GenerarPDF() {
        ...
        try { //Hoja tamaño carta, rotarla (cambiar a horizontal)
            ...
            String strRuta = "C:\\Users\\Teté\\Documents\\GitHub\\gestionProyecto\\4.- Código\\Ventas" + fecha.getFechaActual() + ".pdf";
    ...
    private void agregarTabla(Paragraph parrafo) throws BadElementException 
    {
        ...
        String rotulosColumnas[] = {"Código de Venta", "Fecha de Venta", "Precio Total", "Código de Vendedor"};
...
//        cell.setBackgroundColor (azulClaro);        
        ...
        try {
            ...
            ResultSet resultado = stat.executeQuery("select idVenta,fechaVenta,precioTotal,usuario_idusuario from Venta");

            //Iterar Mientras haya una fila siguiente
            while (resultado.next()) {           //Agregar celdas
                cell = new PdfPCell(new Paragraph(resultado.getInt("idVenta")));
                tabla.addCell(cell);

                cell = new PdfPCell(new Paragraph(resultado.getInt("fechaVenta")));
                tabla.addCell(cell);

                cell = new PdfPCell(new Paragraph(resultado.getInt("precioTotal")));
                tabla.addCell(cell);

                cell = new PdfPCell(new Paragraph(resultado.getInt("usuario_idusuario")));
                tabla.addCell(cell);
